...
def lanzar():
    proceso = entry1.get()
    if(len(proceso)!=0):
        path = 'C:\\Program Files\\Blue Prism Limited\\Blue Prism Automate'
        os.chdir(path)
        comando = 'AutomateC.exe /run "Download BluePrism Process & Object" /sso /startp "<inputs> <input name= \'Proceso\' type= \'text\' value=\''+ proceso +'\' /></inputs>"'
        os.system(comando)
        label2.config( text= proceso + ' Exportado!!', fg='green', font=('helvetica', 12, 'bold'),wraplength=300, justify="center")
    elif(len(proceso)==0):
        label2.config( text= ' Debe ingresar el nombre del proceso u objeto a exportar', fg='red', font=('helvetica', 12, 'bold'),wraplength=300, justify="center")
...
canvas1 = tkinter.Canvas(root, width = 400, height = 375)
canvas1.pack()

root.title("BP Exporter")    

#TITULO

titular = tkinter.Label(root, text='Exportar Objeto o Proceso desde PROD')
titular.config(font=('helvetica', 14))
canvas1.create_window(200, 25, window=titular)

#LABEL DE COMPLETADO O ERROR

label2 = tkinter.Label(root, text='', fg='green', font=('helvetica', 12, 'bold'),wraplength=300, justify="center")
canvas1.create_window(200, 230, window=label2)

#INPUT Y LABEL

label1=tkinter.Label(root, text='Ingrese Proceso u Objeto')
...
button_1 = TkinterCustomButton(text="Exportar", corner_radius=7, command=lanzar)
button_1.place(relx=0.5, rely=0.45, anchor=tkinter.CENTER)

# - start -


connTask =  subprocess.Popen('C:\\Program Files\\Blue Prism Limited\\Blue Prism Automate\\Automate.exe /resourcepc /public',stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE)
